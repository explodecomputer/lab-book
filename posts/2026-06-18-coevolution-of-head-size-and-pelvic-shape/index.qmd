...
The one-sided penalty $\max(0, H_i - P_m)$ captures the asymmetry of the obstetric constraint: when the pelvis is wider than the head ($H_i \leq P_m$) birth is unimpeded and there is no fitness cost; when the head exceeds the pelvis ($H_i > P_m$) fitness falls linearly with the excess.
...
params <- tribble(
  ~s_head, ~s_loco, ~label,
  0.01,     0.02,     "s_head < s_loco (narrowing)",
  0.02,     0.02,     "s_head = s_loco (stasis)",
  0.04,     0.02,     "s_head > s_loco (widening)"
)
params <- lapply(c(20000), \(N) params |> mutate(N=N)) |> bind_rows()

params2 <- params %>%
  mutate(s_head = s_head * 10, s_loco = s_loco * 10, label = paste0(label, " larger s"))

results <- bind_rows(params, params2) |>
  rowwise() |>
  mutate(sim = list(run_sim(s_head = s_head, s_loco = s_loco, N = N))) |>
  unnest(sim)
saveRDS(results, file = here("posts/2026-06-18-coevolution-of-head-size-and-pelvic-shape","results-2.rds"))
```
...
#| fig-height: 7
results <- readRDS(here("posts/2026-06-18-coevolution-of-head-size-and-pelvic-shape/results-2.rds"))
...
Here the `larger s` labelled plots make the s_head and s_loco coefficients same order of magnitude of s_birth, whereas the other plots have s_head and s_loco x10 smaller (meaning s_birth is substantially more important).
...
## Additional examination of the cliff edge model

The cliff edge model implies that as infant head circumference increases beyond birth canal width there is extremely strong negative selection, but the selection coefficient is zero prior to that point. The obstetrical dilemma invokes added parameters - that narrower mother's pelvic width imposes a selective benefit through locomotion, and larger infant head circumferance imposes a selective benefit through cognition. So there is a sharp early life advantage to smaller head / larger pelvic width, but this selective advantage is reversed after birth.

Currently the cliff edge model is implemented with the following shape:

```{r}
#| label: cliff-edge-model
library(dplyr)
library(ggplot2)
library(lattice)
a <- expand.grid(
  infant_head_circumference = seq(0, 10, by = 0.25),
  mother_pelvic_width = seq(0, 10, by = 0.25)) %>%
  mutate(fitness = pmax(0, mother_pelvic_width - infant_head_circumference) * -0.5) %>%
  mutate(rel_fitness = exp(fitness - max(fitness)), rel_fitness = rel_fitness / sum(rel_fitness))
str(a)

b <- expand.grid(
  infant_head_circumference = seq(0, 10, by = 0.25),
  mother_pelvic_width = seq(0, 10, by = 0.25)) %>%
  mutate(fitness = mother_pelvic_width * -0.03) %>%
  mutate(rel_fitness = exp(fitness - max(fitness)), rel_fitness = rel_fitness / sum(rel_fitness))
str(a)

a$component <- "birth"
b$component <- "locomotion"
ab <- rbind(a, b) %>%
  mutate(rel_fitness = exp(fitness - max(fitness)), rel_fitness = rel_fitness / sum(rel_fitness))
...
```{r}
#| label: cliff-edge-wireframe
#| fig-width: 10
wireframe(rel_fitness ~ infant_head_circumference * mother_pelvic_width | ab$component, data = ab, main = "Relative fitness", screen = list(z = 40, x = -60))
```
